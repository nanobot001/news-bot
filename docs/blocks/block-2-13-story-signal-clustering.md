...
- Add a periodic LLM editorial review pass, such as every 15 minutes, that can:
...
- Add a recheck path for recent unmatched, digest-pending, or low-confidence items so they can attach when a better event or cluster appears later.
...
- Keep the implementation auditable and operationally hands-off.
...
- Deterministic logic is good enough to create plausible event buckets and candidate cluster sets for LLM review.
- The periodic LLM review pass can automatically reassign items, merge or split clusters, and create missing clusters without manual approval.
...
- Recent digest-pending, unresolved, or low-confidence items can be rechecked against newer event and story clusters through the periodic review pass.
...
- Automated tests cover sports-style examples, non-sports examples, distinct sub-stories under one broader context, Reddit attachment before low-score filtering, automatic LLM cleanup passes, false-negative inspection, story ledger states, and backwards compatibility.

## Verification

- `npm test`
- `npm run build`
- Run mocked examples for NBA Finals coverage, AI product/policy news, and Toronto restaurant opening/closing coverage through event and story matching and verify the selected cluster, automatic review action, and explanation.
